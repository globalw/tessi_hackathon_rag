...
def main():
    ...
    if os.path.isfile('billability_submission.csv') and os.path.getsize('billability_submission.csv') > 0:
        df_csv.to_csv('billability_submission.csv', mode='a', header=False, index=False)
    else:
        df_csv.to_csv('billability_submission.csv', mode='w', header=True, index=False)
